#!/usr/bin/env python3
"""
QEntL全栈自举生长 v8 - 最终正确版本
===========================================
从v421干净状态开始，每个版本正确添加一个新函数
关键：添加函数时保持def/end平衡
===========================================
"""
import os
...
def add_func_to_qdfs(func_name):
    ...
    if def_count != end_count:
        print(f"  ⚠️ 结构失衡: def={def_count}, end={end_count}")
        # 尝试修复：如果end多了，删除最后一个end
        if end_count > def_count:
            lines = content.split('\n')
            new_lines = []
            skip_end = False
            for i, line in enumerate(lines):
                if line.strip() == 'end' and not skip_end:
                    # 跳过最后一个end
                    if i == len(lines) - 1:
                        continue
            ...
            content = '\n'.join(new_lines)
            end_count -= 1
        print(f"  🔧 已修复: def={def_count}, end={end_count}")
...
if __name__ == '__main__':
    start = int(sys.argv[1]) if len(sys.argv) > 1 else 426
