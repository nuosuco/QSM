...
### 1.4 协同进化规则
**QCL、QVM、QEntL源码三者协同进化。**

正常情况下，只升级QEntL源码（QNS、QDFS、QSM等），QCL和QVM不需要动。

**规则：**
- ✅ **QCL能编译、QVM能运行** → 只升级QEntL源码，QCL/QVM不动
- ❌ **QCL不能编译或QVM不能运行新QEntL源码** → 先升级QCL和QVM源码，自举编译出新的qcl.qbc和qvm.qbc，再用新QCL/QVM去编译QEntL源码

**顺序：出问题才升级QCL+QVM，且先升级QCL+QVM，再升级QEntL源码。**

| 组件 | 类比例子 | 职责 |
|------|------|------|
| **QCL** | 编译器 | 把QEntL源码编译成QBC字节码 |
| **QVM** | 运行时 | 执行QBC字节码 |
| **QEntL** | 源码语言 | 表达功能逻辑 |

**升级顺序：QCL+QVM先 → 再QEntL源码**
...
3. **个性化生长**: 每个用户的升级路径不同
4. **无限迭代**: 没有终点，持续进化
5. **协同进化**: 出问题才升级QCL+QVM，且先升级QCL+QVM再升级QEntL源码
6. **安全回滚**: 永远可以回到任意历史版本
