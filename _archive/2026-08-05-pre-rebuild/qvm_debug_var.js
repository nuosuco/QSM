...
class QVM{
    ...
    run(){
        ...
        while(this.running){
            ...
            this.pc++;
            if(this.debug&&this.cycles<50){
                console.log('cycle='+this.cycles+' pc='+(this.pc-1)+' op=0x'+op.toString(16)+' stack='+JSON.stringify(this.stack.slice(-3)));
            }
            switch(op){
                case 0x00:break;
                case 0x78:{const v=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2;this.stack.push(v);if(this.debug)console.log('  PUSH '+v);break;}
                case 0x79:{const sidx=this.readStrIdx();const s=this.readStrFromPool(sidx);this.stack.push(s);if(this.debug)console.log('  CONST_STR "'+s+'" (sidx='+sidx+')');break;}
                case 0x22:{const n=this.readStrIdx();const v=this.getVar(n);this.stack.push(v);if(this.debug)console.log('  LOAD '+this.readStrFromPool(n)+'='+v);break;}
                case 0x21:{const n=this.readStrIdx();const vn=this.readStrFromPool(n);this.vars[vn]=this.stack.length>0?this.stack.pop():0;if(this.debug)console.log('  STORE '+vn+'='+JSON.stringify(this.vars[vn]));break;}
                case 0x6a:{const n=this.readStrIdx();const vn=this.readStrFromPool(n);this.vars[vn]=this.stack.length>0?this.stack.pop():0;if(this.debug)console.log('  DECL '+vn+'='+JSON.stringify(this.vars[vn]));break;}
                case 0x6f:break;
...
const vm=new QVM(code,spData,true);
vm.extractFunctions();
