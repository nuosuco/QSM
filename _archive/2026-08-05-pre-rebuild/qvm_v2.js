...
builtins.printf=function(a,vm){
    ...
    // Handle %s, %d, %n format specifiers
    let argIdx=1;
    let result='';
...
builtins.file_read=function(a,vm){try{if(a.length>0){const r=fs.readFileSync(a[0],'utf8');console.log('[DBG] file_read OK: '+a[0]+' -> '+r.length+' bytes');return r;}console.log('[DBG] file_read empty args: a='+JSON.stringify(a));}catch(e){console.log('[DBG] file_read error: '+e.message+' stack='+new Error().stack.split('\n').slice(2,6).join('\n'));}return '';};
...
builtins.str_concat=function(a){var r='';for(var i=0;i<a.length;i++)r+=a[i];return r;};
builtins.str_eq=function(a){return a[0]===a[1]?1:0;};
builtins.str_char_at=function(a){var s=a[0],p=a[1]||0;if(typeof s==='string'&&p>=0&&p<s.length)return s[p];return '';};
builtins.int_to_str=function(a){return String(a[0]);};
builtins.len=function(a){if(typeof a[0]==='string')return a[0].length;if(typeof a[0]==='number')return String(a[0]).length;return 0;};
builtins.str_len=function(a){var s=a[0];if(typeof s==='string')return s.length;return 0;};
builtins.str_sub=function(a){var s=a[0],p=a[1]||0,e=a[2]||0;if(typeof s==='string'&&p>=0&&e>p&&e<=s.length)return s.substring(p,e);return '';};
builtins.chr=function(a){var n=a[0];if(typeof n==='number')return String.fromCharCode(n);return '';};
...
class QVM{
    ...
    skipToEnd(){
        ...
        while(this.pc<this.code.length&&depth>0){
            ...
            if(op===0x6c){const cl=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2+cl;depth++;}
            else if(op===0x6e){const cl=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2+cl;depth++;}
    ...
    skipToElseOrEnd(){
        ...
        while(this.pc<this.code.length&&depth>0){
            ...
            else if(op===0x6d){break;} // ELSE: stop skipping, caller handles it
            else if(op===0x6f){if(--depth===0)break;}
        }
    }
    extractFunctions(){
        const funcs={};
        // First pass: find all top-level FUNC_DEFs and their headers
        const topFuncDefs=[];
        for(let i=0;i<this.code.length-10;i++){
            if(this.code[i]===0x66&&this.code[i+7]===0xFF){
                ...
                const params=this.code[i+5]|(this.code[i+6]<<8);
                const cl=this.code[i+3]|(this.code[i+4]<<8);
                if(fname&&fname!==''){
                    topFuncDefs.push({pos:i,name:fname,bodyStart:i+8,params,codeLen:cl});
                    if(!funcs[fname]||funcs[fname].codeLen<cl){
                        funcs[fname]={start:i+8,params,codeLen:cl};
                    }
                    if(fname==='main')console.log('[extract] main func found at start='+(i+8)+' len='+cl);
                }
            }
        }
        // If multiple mains found, use the first one (wrapper from JS compiler)
        const mainDefs = topFuncDefs.filter(d=>d.name==='main');
        if(mainDefs.length > 0){
            const bestMain = mainDefs[0];
            funcs['main'].start = bestMain.bodyStart;
            funcs['main'].name = bestMain.name;
            console.log('[extract] using main at start='+bestMain.bodyStart+' len='+bestMain.codeLen+' name="'+bestMain.name+'"');
        }
        // Compute end positions: each function ends at the next function's position
        for(let i=0;i<topFuncDefs.length;i++){
            const cur=topFuncDefs[i];
            let end;
            if(i+1<topFuncDefs.length){
                // For nested functions, scan to the matching BC_FUNC_END
                let depth=0;
                let j=cur.bodyStart;
                ...
                // Last function: scan to BC_FUNC_END
                let depth=0;
                let j=cur.bodyStart;
                while(j<this.code.length&&depth>=0){
                    const op=this.code[j];
        ...
        // Also register nested functions from the scan
        this.funcs=funcs;
        console.log('[extract] extractFunctions done, found '+Object.keys(funcs).length+' functions');
    }
    run(){
            try{
            // First, find the init code region (before first function definition)
            let initEnd = this.code.length;
            for(let i=0;i<this.code.length-10;i++){
                if(this.code[i]===0x66&&this.code[i+7]===0xFF){
                    initEnd=i;
                    break;
            ...
            // Execute init code if it exists
            if(initEnd>0){
                const savedPc = this.pc;
                this.pc = 0;
                while(this.running && this.pc < initEnd){
                    this.cycles++;
                    const op=this.code[this.pc];
                    this.pc++;
                    switch(op){
                        case 0x78:{const v=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2;this.stack.push(v);break;}
                        case 0x79:{const n=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2;this.stack.push(this.readStrFromPool(n));break;}
                        case 0x22:{const n=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2;const vn=this.readStrFromPool(n);this.stack.push(this.getVar(vn));break;}
                        case 0x21:{const n=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2;const vn=this.readStrFromPool(n);this.vars[vn]=this.stack.length>0?this.stack.pop():0;break;}
                        case 0x6a:{const n=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2;const vn=this.readStrFromPool(n);this.vars[vn]=this.stack.length>0?this.stack.pop():0;break;}
                        case 0x6f:if(this.pc<this.code.length&&this.code[this.pc]===0x6d){this.pc++;this.skipToEnd();}break;
                        case 0x66:this.pc+=7;break;
                        ...
                        case 0x70:{const n=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2;const fn=this.readStrFromPool(n);const nargs=this.code[this.pc++];const args=[];for(let i=0;i<nargs;i++)args.unshift(this.stack.length>0?this.stack.pop():0);if(builtins[fn]){const result=builtins[fn](args,this);if(result!==undefined)this.stack.push(result);}break;}
                        default:break;
                    }
                }
                this.pc = savedPc;
            ...
            while(this.running){
                this.cycles++;
                if(this.cycles>500000)break;
            const op=this.code[this.pc];
            this.pc++;
            if(this.cycles % 50000 === 0) console.log('[QVM] cycle='+this.cycles+' pc='+this.pc+' op='+op);
            switch(op){
                case 0x00:break;
                case 0x78:{const v=this.code[this.pc]|(this.code[this.pc+1]<<8);this.pc+=2;this.stack.push(v);break;}
                ...
                case 0x0e:{const retVal=this.stack.length>0?this.stack.pop():0;if(this.callStack.length>0){const ret=this.callStack.pop();this.vars=ret.vars;this.pc=ret.pc;this.stack.push(retVal);}break;}
                case 0x0d:{const a=this.stack.pop();this.stack.push(a?0:1);break;}
                case 0x0f:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a*b);break;}
                case 0x10:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a+b);break;}
                case 0x11:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a-b);break;}
                case 0x12:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(b!==0?Math.floor(a/b):0);break;}
                case 0x13:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(b!==0?a%b:0);break;}
                ...
                case 0x68:{const idx=this.stack.pop();const arr=this.stack.pop();this.stack.push(arr!==undefined&&arr!==null?arr[idx]||0:0);break;}
                            case 0x69:{const val=this.stack.pop();const idx=this.stack.pop();const arr=this.stack.pop();if(arr!==undefined&&arr!==null)arr[idx]=val;break;}
                ...
                case 0xa9:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a===b?1:0);break;}
                case 0xaa:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a!==b?1:0);break;}
                case 0xab:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a&&b?1:0);break;}
                case 0x6c: {
                ...
                case 0x6e:{
                    ...
                    while(this.pc<condStart+codeLen){
                        ...
                        switch(subOp){
                            ...
                            case 0xaa:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a!==b?1:0);break;}
                            case 0xab:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a&&b?1:0);break;}
                            case 0xac:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a||b?1:0);break;}
                            case 0x20:{const b=this.stack.pop();const a=this.stack.pop();this.stack.push(a&&b?1:0);break;}
                            case 0x1e:{const a=this.stack.pop();this.stack.push(a?0:1);break;}
                            default:this.pc++;
                        }
                    ...
                    const cond=this.stack.length>0?this.stack.pop():0;
                    if(cond){this.pc=bodyStart;}else{this.pc=bodyStart;this.skipToEnd();}
                    break;
                }
                case 0x70:{
                                const sidx = this.readStrIdx();
                                const fname = this.readStrFromPool(sidx);
                                if(fname==='file_read'||fname==='file_write_bytes'||fname==='printf') console.log('[DBG] CALL '+fname+' at pc='+(this.pc-3)+' stack_top='+(this.stack.length>0?this.stack[this.stack.length-1]:'<empty>')+' sp='+this.stack.length);
                    ...
                    if(builtins[fname]){
                        ...
                        if(result!==undefined)this.stack.push(result);
                            ...
                            const nparams=func.params||0;
                            this.callStack.push({pc:this.pc,vars:{...this.vars}});
                            // Push arguments back onto stack for the function to pop
                            for(let i=0;i<Math.min(nargs,nparams);i++){
                ...
                case 0x66:{
                    ...
                    while(j<this.code.length&&depth>0){
                        const op=this.code[j];
                        // 0xFE is BC_FUNC_END only if followed by 0x6F or 0x67
                        if(op===0xFE&&(this.code[j+1]===0x6F||this.code[j+1]===0x67)){depth--;if(depth===0){j+=2;break;}j+=2;}
                        else if(op===0xFE){j++;} // data 0xFE, skip 1 byte
                        else if(op===0x66&&this.code[j+7]===0xFF){j+=8;depth++;}
                        else if(op===0x67){j++;if(depth<=0)break;}
                        else if(op===0x6c){const cl=this.code[j+1]|(this.code[j+2]<<8);j+=3+cl;}
                        else if(op===0x6e){const cl=this.code[j+1]|(this.code[j+2]<<8);j+=3+cl;}
...
if(!mainFunc){
    // Use first function as main (wrapper from JS compiler)
    const firstFunc=vm.firstFunc;
    if(firstFunc){
        vm.pc = firstFunc + 8;
        console.log('[QVM] 未找到"main"，使用第一个函数作为main (位置 ' + firstFunc + ')');
    } else {
        console.log('错误: 未找到任何函数');
    }
} else {
    // Skip FUNC_DEF header (8 bytes) to get to actual function body
    vm.pc = mainFunc.start + 8;
    console.log('[QVM] 从位置 ' + (mainFunc.start + 8) + ' 开始执行 (跳过FUNC_DEF header)');
}
vm.run();