...
JIEQI_DATA = [
    ...
    {"name": "立秋", "month": 8, "day": 8, "season": "秋", "desc": "秋季开始，暑去凉来",
...
def get_current_jieqi(dt: datetime = None) -> dict:
    """获取当前节气"""
    if dt is None:
        dt = datetime.now()
    ...
    if not current:
        current = JIEQI_DATA[-1]  # 冬至兜底
        next_jq = JIEQI_DATA[0]

    return {"current": current, "next": next_jq}


def get_jieqi_advice(dt: datetime = None) -> dict:
    """获取当前节气的养生建议"""
    result = get_current_jieqi(dt)
    jq = result["current"]
    next_jq = result["next"]
    ...
    return {
        ...
        "tea": jq["tea"],
        "avoid": jq["avoid"],
        "next_jieqi": next_jq["name"],
        "next_date": f"{next_jq['month']}月{next_jq['day']}日",
    }


def get_all_jieqi() -> list:
    """获取全部二十四节气列表"""
    return [{"name": jq["name"], "month": jq["month"], "day": jq["day"],
             "season": jq["season"], "desc": jq["desc"]} for jq in JIEQI_DATA]
