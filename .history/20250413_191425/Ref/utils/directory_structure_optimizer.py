...
class DirectoryStructureOptimizer:
    ...
    def __init__(self, project_root: str):
        ...
        self.backup_dir = os.path.join(self.project_root, 'Ref', 'backup', 'structure_optimizer')
        ...
        if guardian_available:
            self.guardian = get_guardian(self.project_root)
        else:
            self.guardian = None
        ...
        logger.info(f"目录结构优化器初始化完成，项目根目录: {self.project_root}")
    
    def create_standard_directory_structure(self) -> Dict[str, Any]:
        """
        创建标准目录结构
        ...
        for module in self.main_modules:
            ...
            # 为每个模块创建标准子目录
            subdirs = self.standard_subdirs.copy()
            
            # WeQ有额外的特殊目录
            if module == 'WeQ':
                subdirs.extend(self.weq_special_dirs)
                ...
                if os.path.exists(train_path):
                    ...
                    for subdir in ['helpers', 'models', 'data']:
                        ...
                        self._create_directory(train_subdir, results)
            
            # 创建标准子目录
            for subdir in subdirs:
                subdir_path = os.path.join(module_path, subdir)
                self._create_directory(subdir_path, results)
    ...
    def _create_directory(self, dir_path: str, results: Dict[str, List[str]]):
        """创建目录并更新结果"""
    ...
    def organize_files(self, module: str, dry_run: bool = True) -> Dict[str, Any]:
        """
        组织指定模块的文件
        ...
        for item in os.listdir(module_path):
            ...
            # 分析文件用途
            target_dir = self._determine_file_target_directory(module, item_path)
            if target_dir:
                ...
                if not dry_run:
                    try:
                        ...
                        if item.endswith('.py'):
                            self._update_imports_and_move(item_path, target_path, module, results)
                        else:
                            # 直接移动非Python文件
                            self._backup_and_move_file(item_path, target_path, results)
                    except Exception as e:
    ...
    def _determine_file_target_directory(self, module: str, file_path: str) -> Optional[str]:
        """确定文件应该移动到的目标目录"""
    ...
    def _update_imports_and_move(self, source_path: str, target_path: str, module: str, results: Dict):
        ...
        try:
            ...
            # 更新相对导入语句
            new_content = self._adjust_import_statements(content, source_path, target_path, module)
    ...
    def _adjust_import_statements(self, content: str, source_path: str, target_path: str, module: str) -> str:
        """调整Python文件中的导入语句"""
        # 获取源文件和目标文件的相对路径
    ...
    def _backup_and_move_file(self, source_path: str, target_path: str, results: Dict):
        """备份文件并移动到新位置"""
        try:
            # 备份文件
            backup_path = self._backup_file(source_path)
    ...
    def analyze_project_structure(self) -> Dict[str, Any]:
        ...
        for module in self.main_modules:
            ...
            report['modules'][module] = self._analyze_module(module_path)
            ...
            # 生成建议
            self._generate_recommendations(module, report)
    ...
    def _analyze_module(self, module_path: str) -> Dict[str, Any]:
        ...
        # 检查标准子目录
        standard_dirs = self.standard_subdirs
        if module_name == 'WeQ':
            standard_dirs = standard_dirs + self.weq_special_dirs
    ...
    def _generate_recommendations(self, module: str, report: Dict[str, Any]):
        """根据模块分析生成优化建议"""
        ...
        if module == 'WeQ' and 'train' in module_report['subdirectories']:
            train_path = os.path.join(self.project_root, module, 'train')
            if not os.path.exists(os.path.join(train_path, 'helpers')):
                report['recommendations'].append(
                    f"在 WeQ/train 目录中创建 helpers 子目录，用于存放训练辅助脚本"
                )
    
    def generate_optimization_report(self, output_path: str = None) -> Dict[str, Any]:
        """
        生成优化报告并可选保存到文件
        ...
        Returns:
            优化报告
        """
        report = self.analyze_project_structure()
...
def get_directory_optimizer(project_root: str = None) -> DirectoryStructureOptimizer:
    """
    获取目录结构优化器的单例实例
