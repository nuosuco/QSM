...
class EnhancedQuantumNetwork:
    ...
    def __init__(self):
        self.registry_path = "Ref/data/quantum_gene_registry.json"
        self.entanglement_registry_path = "Ref/data/quantum_entanglement_registry.json"
        self.registry = self._load_registry()
        self.entanglement_registry = self._load_entanglement_registry()
        self.log_path = "Ref/logs/quantum_network_log.json"
        ...
        # 已连接的文件计数
        self.connected_files = 0
        self.connection_count = 0
        
        # 确保必要的目录存在
        os.makedirs(os.path.dirname(self.registry_path), exist_ok=True)
        os.makedirs(os.path.dirname(self.entanglement_registry_path), exist_ok=True)
    ...
    def _load_entanglement_registry(self) -> Dict:
        """加载量子纠缠注册表"""
        if os.path.exists(self.entanglement_registry_path):
            try:
                with open(self.entanglement_registry_path, 'r', encoding='utf-8') as f:
                    return json.load(f)
            except Exception as e:
                print(f"加载纠缠注册表失败: {e}")
                return {"channels": {}}
    ...
    def _save_entanglement_registry(self) -> None:
        """保存量子纠缠注册表"""
        try:
            with open(self.entanglement_registry_path, 'w', encoding='utf-8') as f:
                json.dump(self.entanglement_registry, f, indent=2, ensure_ascii=False)
        except Exception as e:
            print(f"保存纠缠注册表失败: {e}")
    
    def _save_network_log(self, log_data: Dict) -> None:
        """保存网络日志"""
        try:
    ...
    def extract_gene_from_file(self, file_path: str) -> str:
        """从文件中提取量子基因编码"""
        try:
            ...
            if match:
                ...
                for file_key, info in self.registry.get("files", {}).items():
    ...
    def create_entanglement_channel(self, source_gene: str, target_gene: str, channel_type: str = "SYNC") -> str:
        ...
        # 检查是否已存在
        for existing_id, channel in self.entanglement_registry["channels"].items():
        ...
        if self.connection_count % 50 == 0:  # 每创建50个连接保存一次
            self._save_entanglement_registry()
            
        return channel_id
    
    def select_key_file(self, directory: str) -> str:
        """
        从目录中选择最重要的文件
    ...
    def find_key_files_in_project(self) -> Dict[str, str]:
        ...
        for root, dirs, _ in os.walk(project_root):
            ...
            # 为当前目录选择关键文件
            key_file = self.select_key_file(root)
            if key_file:
                gene = self.extract_gene_from_file(key_file)
                if gene:
                    key_files[key_file] = gene
        
        return key_files
    
    def build_mesh_network(self, key_files: Dict[str, str]) -> None:
        """
        构建网格型量子纠缠网络
        ...
        for i, (source_file, source_gene) in enumerate(file_list):
            ...
            for j in closest_indices:
                ...
                channel_id = self.create_entanglement_channel(source_gene, target_gene)
                if channel_id:
                    connections.append({
                        "target_file": target_file,
                        "channel_id": channel_id
                    })
                    self.connected_files += 1
        ...
        # 保存最终的纠缠注册表和日志
        self._save_entanglement_registry()
        self._save_network_log(log_data)
        
        print(f"增强型量子纠缠网络构建完成!")
        print(f"- 关键文件总数: {len(key_files)}")
        print(f"- 创建的量子纠缠信道总数: {self.connection_count}")
        print(f"- 网络日志已保存到: {self.log_path}")
    
    def build_enhanced_network(self) -> None:
        """构建增强型量子纠缠网络"""
        # 步骤1: 找到每个目录的关键文件
        key_files = self.find_key_files_in_project()
        print(f"已识别 {len(key_files)} 个关键文件")
        
        # 步骤2: 构建网格网络
        self.build_mesh_network(key_files)
