...
class SomEconomyModel:
    ...
    def create_som_coin(self, amount: float, recipient_id: str) -> Optional[QuantumAsset]:
        """创建松麦币
        ...
        # 分配给松麦链
        if self.chain:
            som_coin.assign_to_chain(self.chain.chain_id)
    ...
    def process_organic_purchase(self, user_id: str, product_id: str, purchase_amount: float) -> Dict:
        ...
        # 创建松麦币
        som_coin = self.create_som_coin(reward_amount, user_id)
    ...
    def register_new_user(self, user_id: str, user_data: Dict) -> Dict:
        ...
        # 创建松麦币
        som_coin = self.create_som_coin(initial_reward, user_id)
