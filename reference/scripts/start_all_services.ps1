...
"@
            ...
            # 修改main函数处理--monitor参数 - 拆分替换操作避免复杂正则
            $mainPattern = 'if __name__ == ''__main__'':'
            if ($content -match $mainPattern) {
                $content = $content -replace $mainPattern, "$mainPattern`n    parser = argparse.ArgumentParser(description='量子基因标记工具')"
            }
            ...
            if (-not ($content -match "--monitor")) {
                ...
                if ($content -match $helpArgPattern) {
                    ...
                    # 如果找不到help参数，在靠前位置添加monitor参数
                    $addArgumentPattern = "parser\.add_argument\("
                    if ($content -match $addArgumentPattern) {
                        $content = $content -replace $addArgumentPattern, "parser.add_argument('--monitor', action='store_true', help='启动监控模式')`n    parser.add_argument("
                    }
                }
            }
