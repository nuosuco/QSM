...
def is_redundant_file(file_path):
    """检查文件是否为冗余文件"""
...
def find_redundant_files(directory=None):
    """查找冗余文件"""
...
def find_redundant_dirs(directory=None):
    """查找冗余目录"""
    if directory is None:
        directory = ROOT_DIR
...
def merge_directory(source_dir, target_dir):
...
def update_imports_in_file(file_path, directory_merges):
    """更新文件中的导入路径"""
...
def update_imports(directory_merges):
    """更新所有Python文件中的导入路径"""
    logger.info("更新导入路径...")
...
def print_plan(redundant_files, redundant_dirs, directory_merges):
    """打印清理计划"""
    print("\n===== 项目结构清理计划 =====")
...
def execute_cleanup(redundant_files, redundant_dirs, directory_merges):
    """执行清理操作"""
