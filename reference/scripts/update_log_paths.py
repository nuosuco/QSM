...
def needs_log_config_import(content):
...
def add_log_config_import(content):
    """添加log_config模块导入"""
    lines = content.split('\n')
    import_idx = -1
...
def suggest_log_config_usage(file_path, content):
...
def process_py_file(file_path, dry_run=False):
    """处理单个Python文件，更新日志路径"""
    try:
