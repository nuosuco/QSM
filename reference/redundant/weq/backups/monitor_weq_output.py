...
def notify_weq_monitor(file_path: str, change_type: str, old_path: str = None) -> bool:
    """
    通知WeQ输出监控系统文件变更
...
class WeQOutputMonitor:
    ...
    def __init__(self, weq_dirs: List[str] = None, scan_interval: int = 30,
        ...
        # 获取量子基因标记器和文件监控器实例
        self.gene_marker = get_gene_marker()
        self.file_monitor = get_file_monitor()
        
        # 设置WeQ输出目录
        if weq_dirs:
            self.weq_dirs = weq_dirs
        else:
            # 获取默认的WeQ输出目录
            self.weq_dirs = getattr(self.gene_marker, 'WEQ_OUTPUT_DIRS', [])
            
            # 确保目录是绝对路径
            project_root = self.gene_marker.get_project_root()
            self.weq_dirs = [os.path.join(project_root, dir_path) if not os.path.isabs(dir_path) else dir_path
                            for dir_path in self.weq_dirs]
        
        # 如果目录列表为空，添加默认目录
        if not self.weq_dirs:
            self.weq_dirs = [
        ...
        if backup_dir:
            ...
            self.backup_dir = os.path.join(project_root, "WeQ", "backup")
        ...
        # 设置为全局实例
        global _weq_monitor_instance
        _weq_monitor_instance = self
        
        self.logger.info(f"初始化WeQ输出监控器，监控目录: {self.weq_dirs}")
    ...
    def start(self):
        ...
        if not self.file_monitor:
            self.logger.error("无法获取文件监控器实例")
            return False
            
        # 如果文件监控器未运行，启动它
        if not getattr(self.file_monitor, 'running', False):
            try:
                self.file_monitor.start()
                self.logger.info("启动文件监控器")
            except Exception as e:
        ...
        # 添加WeQ输出目录到文件监控器
        try:
            self.file_monitor.monitor_weq_directories(self.weq_dirs)
            self.logger.info("已将WeQ输出目录添加到文件监控器")
        except Exception as e:
            self.logger.error(f"添加WeQ输出目录到文件监控器失败: {e}")
            
        # 执行初始扫描
        self._scan_weq_output()
        
        self.logger.info("WeQ输出监控器启动成功")
        return True
    
    def stop(self):
        """停止监控"""
        self.running = False
        self.logger.info("WeQ输出监控器已停止")
        return True
    ...
    def run_continuously(self):
        ...
        try:
            ...
            self.logger.info("接收到键盘中断，停止监控...")
        finally:
            self.stop()
            
        return True
    
    def _scan_weq_output(self):
        """扫描WeQ输出目录并处理文件"""
        if not self.running:
            return 0
            
        start_time = time.time()
        self.logger.info("开始扫描WeQ输出目录...")
        ...
        for dir_path in self.weq_dirs:
            if not os.path.exists(dir_path):
                self.logger.warning(f"WeQ输出目录不存在: {dir_path}")
                continue
            ...
            for root, dirs, files in os.walk(dir_path):
                ...
                for file in files:
                    ...
                    if ext.lower() in weq_file_extensions:
                        ...
                        # 处理文件
                        if self._process_weq_file(file_path):
                            processed_count += 1
        ...
        self.statistics["files_processed"] += processed_count
        self.statistics["last_scan_time"] = time.time()
        
        self.logger.info(f"扫描完成，处理了 {processed_count} 个文件，耗时 {scan_duration:.2f} 秒")
        return processed_count
    
    def _process_weq_file(self, file_path: str) -> bool:
        """
        处理WeQ输出文件
        ...
        try:
            ...
            # 检查是否有量子基因标记
            has_marker = self.gene_marker.has_quantum_gene_marker(file_path)
            
            if not has_marker:
                # 尝试添加标记
                entangled_objects = self._suggest_weq_entangled_objects(file_path)
                
                if self.gene_marker.add_quantum_gene_marker(file_path, entangled_objects, notify=False):
                    self.statistics["markers_added"] += 1
                    self.logger.info(f"为WeQ输出文件添加了量子基因标记: {file_path}")
                    ...
                    if self.enable_redundancy:
                        self._create_redundancy(file_path)
                        
                    return True
            else:
                # 验证现有标记
                result = self._verify_weq_file(file_path)
                
                if result:
                    self.statistics["markers_updated"] += 1
                    
                    # 通知其他监控系统
                    self.notify_monitoring_systems(file_path, 'update')
            ...
            self.logger.error(f"处理WeQ输出文件时出错: {file_path} - {str(e)}")
            self.logger.debug(traceback.format_exc())
            return False
    
    def _suggest_weq_entangled_objects(self, file_path: str) -> List[str]:
        """
        为WeQ输出文件推断潜在的纠缠对象
        ...
        try:
            ...
            for pattern in patterns:
                for match in re.finditer(pattern, content):
                    ...
                    elif os.path.exists(os.path.join(self.gene_marker.get_project_root(), norm_path)):
                        entangled_objects.append(os.path.join(self.gene_marker.get_project_root(), norm_path))
            
            return list(set(entangled_objects))
        except Exception as e:
            self.logger.error(f"推断WeQ输出文件纠缠对象时出错: {file_path} - {str(e)}")
            return []
    
    def _verify_weq_file(self, file_path: str) -> bool:
        """
        验证WeQ输出文件的纠缠对象
        ...
        try:
            ...
            # 获取当前纠缠对象
            current_objects = self.gene_marker._parse_entangled_objects(content)
            
            # 推断潜在纠缠对象
            potential_objects = self._suggest_weq_entangled_objects(file_path)
            ...
            if need_update:
                ...
                # 更新标记
                result = self.gene_marker.update_quantum_gene_marker(file_path, merged_objects)
                
                if result:
                    self.logger.info(f"更新了WeQ输出文件的纠缠对象: {file_path}")
            ...
            self.logger.error(f"验证WeQ输出文件时出错: {file_path} - {str(e)}")
            self.logger.debug(traceback.format_exc())
            return False
    ...
    def _backup_file(self, file_path: str) -> bool:
        ...
        try:
            # 创建备份文件名
            rel_path = os.path.relpath(file_path, self.gene_marker.get_project_root())
            ...
            if os.path.exists(backup_path):
                ...
                if source_mtime > backup_mtime:
                    ...
                    self.logger.debug(f"更新文件备份: {file_path} -> {backup_path}")
            else:
                # 创建新备份
                import shutil
                shutil.copy2(file_path, backup_path)
                self.statistics["backups_created"] += 1
                self.logger.debug(f"创建文件备份: {file_path} -> {backup_path}")
                
            return True
        except Exception as e:
            self.logger.error(f"备份文件时出错: {file_path} - {str(e)}")
            return False
    
    def _create_redundancy(self, file_path: str) -> bool:
        """
        为文件创建冗余引用
        ...
        """
        try:
            if not self.gene_marker.has_quantum_gene_marker(file_path):
                return False
                
            result = self.gene_marker.create_redundant_references(file_path, self.min_redundancy)
            
            if result:
                self.statistics["redundancy_created"] += 1
                self.logger.info(f"为文件创建了冗余引用: {file_path}")
                
            return result
        except Exception as e:
            self.logger.error(f"创建冗余引用时出错: {file_path} - {str(e)}")
            return False
    ...
    def is_weq_output_file(self, file_path: str) -> bool:
        """
        检查文件是否是WeQ输出文件
    ...
    def notify_monitoring_systems(self, file_path: str, change_type: str, old_path: str = None):
        """
        ...
        # 通知量子基因标记系统
        try:
            if self.gene_marker and hasattr(self.gene_marker, 'notify_monitoring_systems'):
                self.gene_marker.notify_monitoring_systems(file_path, change_type, old_path)
                self.logger.info(f"通知量子基因标记系统: {change_type} {file_path}")
        except Exception as e:
            self.logger.error(f"通知量子基因标记系统失败: {str(e)}")
            self.logger.debug(traceback.format_exc())
            success = False
            
        # 通知文件监控系统
        try:
            if self.file_monitor and hasattr(self.file_monitor.event_handler, 'notify_marker_monitor'):
                self.file_monitor.event_handler.notify_marker_monitor(file_path, change_type, old_path)
                self.logger.info(f"通知文件监控系统: {change_type} {file_path}")
        except Exception as e:
            self.logger.error(f"通知文件监控系统失败: {str(e)}")
            self.logger.debug(traceback.format_exc())
            success = False
