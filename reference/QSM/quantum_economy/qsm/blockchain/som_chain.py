...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("qsm_chain.log"),
...
class SomEconomyChain:
    ...
    def create_qsm_coin(self, amount: float, creator_id: str = "system") -> str:
        """创建松麦币
        ...
        # 更新经济状态
        self.economy_state["qsm_coin"]["total_supply"] += amount
        self.economy_state["qsm_coin"]["circulating_supply"] += amount
        self.economy_state["qsm_coin"]["last_update"] = datetime.datetime.now().isoformat()
    ...
    def transfer_qsm_coin(self, from_id: str, to_id: str, amount: float, memo: str = "") -> str:
        """转移松麦币
    ...
    def process_purchase(self, user_id: str, product_id: str, quantity: int, payment_method: str = "qsm_coin") -> str:
        """处理购买交易
    ...
    def adjust_qsm_coin_value(self) -> float:
        """调整松麦币价值
        ...
        # 获取流通中的松麦币供应量
        circulating_supply = self.economy_state["qsm_coin"]["circulating_supply"]
        
        # 简单的价值调整算法，实际应用中可以使用更复杂的经济模型
        if circulating_supply > 0:
            activity_factor = transaction_volume / circulating_supply
            current_value = self.economy_state["qsm_coin"]["current_value"]
            ...
            # 更新价值
            self.economy_state["qsm_coin"]["current_value"] = new_value
            self.economy_state["qsm_coin"]["last_update"] = datetime.datetime.now().isoformat()
            ...
            return self.economy_state["qsm_coin"]["current_value"]
    ...
    def mine_block(self) -> Dict:
        ...
        # 经济子链的每个区块会自动调整松麦币价值
        self.adjust_qsm_coin_value()
        
        logger.info(f"挖掘新区块: {block['block_id']}, 包含 {len(block['transactions'])} 笔交易")
        return block
    
    def get_qsm_coin_info(self) -> Dict:
        """获取松麦币信息
        
        Returns:
            松麦币信息
        """
        return self.economy_state["qsm_coin"]
