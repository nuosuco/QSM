...
# 导入松麦币发行系统
from quantum_economy.QSM.coin.QSM_coin import SomCoinEmission, SomCoinRewardDistributor
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("qsm_coin_api.log"),
...
# 创建蓝图
qsm_coin_api = Blueprint('qsm_coin_api', __name__)
...
def get_emission_system():
    ...
    if hasattr(current_app, 'qsm_coin_emission'):
        return current_app.qsm_coin_emission
    
    # 如果还没有初始化，则创建一个新的实例
    # 注意：这是一个简化的处理方式，实际应用中应确保只有一个实例
    logger.warning("松麦币发行系统未在应用上下文中初始化，创建新实例")
    emission_system = SomCoinEmission()
    current_app.qsm_coin_emission = emission_system
    return emission_system

# 获取应用上下文中的松麦币奖励分配器
def get_reward_distributor():
    if hasattr(current_app, 'qsm_coin_reward_distributor'):
        return current_app.qsm_coin_reward_distributor
    
    emission_system = get_emission_system()
    reward_distributor = SomCoinRewardDistributor(emission_system)
    current_app.qsm_coin_reward_distributor = reward_distributor
    return reward_distributor

# API路由：获取松麦币信息
@qsm_coin_api.route('/info', methods=['GET'])
def get_coin_info():
    """获取松麦币基本信息"""
    try:
        emission_system = get_emission_system()
        ...
        info = {
            ...
            "symbol": "SOM",
            "decimals": 8,
            "logo": "/static/images/qsm-coin-logo.png",
            "description": "松麦生态系统的核心通证，基于量子区块链技术",
            "website": "https://qsm.example.org",
...
# API路由：获取松麦币经济参数
@qsm_coin_api.route('/parameters', methods=['GET'])
...
# API路由：获取松麦币发行统计
@qsm_coin_api.route('/emission-stats', methods=['GET'])
...
# API路由：获取用户余额
@qsm_coin_api.route('/balance/<address>', methods=['GET'])
...
# API路由：获取用户交易历史
@qsm_coin_api.route('/transactions/<address>', methods=['GET'])
...
# API路由：创建转账交易
@qsm_coin_api.route('/transfer', methods=['POST'])
@require_auth
def create_transfer():
    """创建松麦币转账交易"""
    try:
        ...
        if emission_system.economy_chain:
            # 使用经济链进行转账
            try:
                tx_id = emission_system.economy_chain.transfer_qsm_coin(
...
# API路由：提交贡献数据
@qsm_coin_api.route('/contribute', methods=['POST'])
@require_auth
...
# API路由：请求特定贡献的奖励
@qsm_coin_api.route('/reward', methods=['POST'])
@require_auth
def request_reward():
    """为特定贡献请求松麦币奖励"""
    try:
...
# API路由：分配团队奖励
@qsm_coin_api.route('/distribute-rewards', methods=['POST'])
@require_auth
