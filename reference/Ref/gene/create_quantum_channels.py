...
class QuantumChannelCreator:
    ...
    def create_channels_for_core_docs(self) -> None:
        """为核心文档创建量子纠缠信道"""
        # 定义核心文档路径
        core_docs = [
        ...
        for doc_path in core_docs:
            gene = self.extract_gene_from_file(doc_path)
            if gene:
        ...
        for code_path in code_files:
            gene = self.extract_gene_from_file(code_path)
            if gene:
        ...
        for doc_path, doc_gene in doc_genes.items():
            for code_path, code_gene in code_genes.items():
                channel_id = self.create_entanglement_channel(doc_gene, code_gene, "DATA")
                if channel_id:
        ...
        # 打印量子纠缠网络可视化
        self._print_entanglement_network(doc_genes, code_genes)
    
    def _print_entanglement_network(self, doc_genes: Dict[str, str], code_genes: Dict[str, str]) -> None:
        """打印量子纠缠网络可视化"""
        print("\n=== 量子纠缠网络 ===")
        print("文档网络:")
