...
from .file_integrity_monitor import get_monitor, FileIntegrityMonitor
...
class FileOrganizationGuardian:
    ...
    def __init__(self, 
        ...
        # 获取文件完整性监控器
        full_registry_path = os.path.join(self.workspace_root, registry_path)
        self.monitor = get_monitor(full_registry_path)
        
        # 组件目录映射
        self.component_dirs = {
            'QSM': os.path.join(self.workspace_root, 'QSM'),
            'SOM': os.path.join(self.workspace_root, 'SOM'),
            'Ref': os.path.join(self.workspace_root, 'Ref'),
            'WeQ': os.path.join(self.workspace_root, 'WeQ')
        }
        
        # 资源目录映射
        self.resource_dirs = {
            'static': os.path.join(self.workspace_root, 'static'),
            'templates': os.path.join(self.workspace_root, 'templates'),
            'global': os.path.join(self.workspace_root, 'global')
        }
        
        logger.info(f"文件组织监护器初始化完成，工作区: {self.workspace_root}")
    ...
    def register_existing_files(self, 
        ...
        if not directories:
            # 默认注册所有主要组件目录
            directories = list(self.component_dirs.values()) + list(self.resource_dirs.values())
            ...
            for root, dirs, files in os.walk(directory):
                for file in files:
                    ...
                    try:
                        ...
                        # 尝试确定文件用途
                        purpose = self._guess_file_purpose(filepath, content)
                        
                        # 尝试确定文件依赖
                        dependencies = self._guess_file_dependencies(filepath, content)
    ...
    def _guess_file_purpose(self, filepath: str, content: str) -> str:
        """
        猜测文件用途
        ...
        # 基于组件名称的推测
        for component, path in self.component_dirs.items():
            if filepath.startswith(path):
                relative_path = os.path.relpath(filepath, path)
                return f"{component}组件 {relative_path} 文件"
        
        # 默认
        return f"文件: {os.path.relpath(filepath, self.workspace_root)}"
    
    def _guess_file_dependencies(self, filepath: str, content: str) -> List[str]:
        """
        猜测文件依赖关系
        ...
        if ext == '.py':
            ...
            for module in from_imports:
                ...
                if '.' in module:
                    parts = module.split('.')
                    possible_path = os.path.join(self.workspace_root, *parts) + '.py'
            ...
            for link in css_links:
                if link.startswith('/'):
                    # 绝对路径
                    possible_path = os.path.join(self.workspace_root, link.lstrip('/'))
                    if os.path.exists(possible_path):
                        dependencies.append(possible_path)
    ...
    def safe_create_file(self, 
        ...
        # 检查文件冲突
        conflict_result = self.monitor.check_conflicts(filepath, content)
    ...
    def safe_edit_file(self, 
        ...
        # 检查文件是否已注册
        if filepath not in self.monitor.file_registry:
            logger.warning(f"文件 {filepath} 未注册，将先进行注册")
            try:
                with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                    old_content = f.read()
                
                purpose = self._guess_file_purpose(filepath, old_content)
                dependencies = self._guess_file_dependencies(filepath, old_content)
        ...
        try:
            ...
            # 更新注册信息
            purpose = self.monitor.file_registry[filepath].get('purpose', '')
            dependencies = self.monitor.file_registry[filepath].get('dependencies', [])
            
            # 可能的依赖变更
            new_dependencies = self._guess_file_dependencies(filepath, new_content)
    ...
    def safe_delete_file(self, 
        ...
        if filepath in self.monitor.file_registry:
            dependents = []
            
            for reg_path, reg_info in self.monitor.file_registry.items():
        ...
        try:
            ...
            # 从注册表中移除
            if filepath in self.monitor.file_registry:
                del self.monitor.file_registry[filepath]
    ...
    def scan_project_and_report(self) -> Dict[str, Any]:
        """
        扫描整个项目并生成报告
        ...
        # 扫描每个组件目录
        for component, directory in self.component_dirs.items():
            if os.path.exists(directory):
                scan_result = self.monitor.scan_directory(directory)
        ...
        # 扫描资源目录
        resource_stats = {}
        for res_type, directory in self.resource_dirs.items():
            if os.path.exists(directory):
                scan_result = self.monitor.scan_directory(directory)
        ...
        # 获取组织建议
        org_suggestions = self.monitor.suggest_organization()
    ...
    def enforce_project_standards(self, auto_fix: bool = False) -> Dict[str, Any]:
        """
        ...
        self._check_global_consistency(results, auto_fix)
        
        # 检查2：确保static和templates目录的组织正确
        self._check_resource_dirs(results, auto_fix)
        
        # 检查3：检查quantum_blockchain目录的重复
        self._check_duplicate_modules(results, auto_fix, 'quantum_blockchain')
        
        # 检查4：检查base.html等基础模板的一致性
        self._check_template_consistency(results, auto_fix)
        
        # 检查5：检查非必要的备份文件(.bak, .old等)
        self._check_backup_files(results, auto_fix)
        
        return results
    
    def _check_global_consistency(self, results: Dict, auto_fix: bool):
        """检查全局资源的一致性"""
        main_global = self.resource_dirs['global']
        ...
        # 检查每个组件中的global目录
        for component, comp_dir in self.component_dirs.items():
            ...
            for root, _, files in os.walk(comp_global):
                for file in files:
                    ...
                    if os.path.exists(main_file):
                        ...
                        similarity = self.monitor._calculate_content_similarity(comp_content, main_content)
    ...
    def _check_resource_dirs(self, results: Dict, auto_fix: bool):
        """检查资源目录的组织"""
    ...
    def _check_duplicate_modules(self, results: Dict, auto_fix: bool, module_name: str):
        """检查重复模块"""
        module_dirs = []
    ...
    def _check_template_consistency(self, results: Dict, auto_fix: bool):
        """检查模板文件的一致性"""
        base_templates = {}
        ...
        if len(base_templates) > 1:
            ...
            for comp1, data1 in base_templates.items():
                for comp2, data2 in base_templates.items():
                    ...
                    similarity = self.monitor._calculate_content_similarity(
                        data1['content'], 
                        data2['content']
                    )
    ...
    def _check_backup_files(self, results: Dict, auto_fix: bool):
        """检查备份文件"""
...
def get_guardian(workspace_root: str = None, 
                registry_path: str = 'Ref/data/file_registry.json',
