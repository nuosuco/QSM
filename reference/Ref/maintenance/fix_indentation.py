...
def fix_indentation(target_file):
    """修复文件中的缩进问题"""
    try:
        # 读取文件内容
        ...
        for line in lines:
            ...
            # 如果在方法内且行首缩进小于预期
            if in_method and line.strip() and not line.startswith(expected_indent) and not re.match(r'\s+def \w+\(', line) and not re.match(r'\s+"""', line):
                # 如果是一个新的函数定义或类定义，重置状态
                if re.match(r'\s+def \w+\(', line) or re.match(r'class \w+', line):
