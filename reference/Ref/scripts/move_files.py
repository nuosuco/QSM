...
def determine_target_directory(filepath, module_name):
    """
    确定文件应该移动到的目标目录
...
def move_file(source_path, target_dir, dry_run=True):
    """
    移动文件并更新导入路径
...
def organize_module(module_name, dry_run=True):
    """
    组织模块文件
...
def main():
    ...
    parser = argparse.ArgumentParser(description="项目文件组织工具")
    parser.add_argument('module', help='要组织的模块名称')
    parser.add_argument('--apply', action='store_true', help='实际执行文件移动（默认为模拟模式）')
    
    args = parser.parse_args()
    
    # 验证模块名称
    valid_modules = {'QSM', 'WeQ', 'SOM', 'Ref', 'QEntL'}
    if args.module not in valid_modules:
        print(f"错误: 无效的模块名称 {args.module}")
        print(f"有效的模块名称: {', '.join(valid_modules)}")
        return
    
    # 组织模块
    organize_module(args.module, not args.apply)
