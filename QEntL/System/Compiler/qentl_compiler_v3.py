...
class Parser:
    ...
    def _parse_primary(self):
        ...
        if t.type == TokenType.NUMBER_LIT:
            ...
            builtin_funcs = {'打印', '长度', '推入', '弹出', '类型', '绝对值', '最大值', '最小值', '字典', '翻转', '包含', '连接', '分割', '替换', '子串', '增1', '减1', '取整', '幂', '是数字', '是字母', '是空', '格式', '查找', '删除', '插入', '重复', '排序', '去重', '范围数', '随机数', '大写', '小写', '首大写', '计数', '开始以', '结束以', '键', '值', '合并', '四舍五入', '浮点', '平方', '平方根', '求和', '求积', '读取文件', '写入文件', '文件存在', '列出目录', '创建目录', '删除文件', '当前时间', '获取环境', '执行命令', '等待', '加载模块', '运行QBC', '字符位置', '删除空白', '左边空白', '重复到'}
