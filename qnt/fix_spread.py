...
def get_spread_data(exchange_name, symbol):
    ...
    try:
        ...
        spot_price = float(spot_ticker.get('last', 0)) if spot_ticker else 0
        
        # 获取永续价格
        perp_symbol = f"{symbol.split('/')[0]}:USDT"
        perp_ticker = exchange.fetch_ticker(perp_symbol)
        perp_price = float(perp_ticker.get('last', 0)) if perp_ticker else 0
        ...
        print(f"  ❌ {exchange_name} {symbol} 失败: {e}")
