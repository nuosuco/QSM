#define _GNU_SOURCE
/*
 * qvm_bootstrap_opt.c — QVM量子虚拟机 C语言启动器（优化版）
...
#define MAX_OPS 4096  /* 优化：小buffer，降低内存占用 */
#define OPT_MMAP 1
...
int main(int argc, char *argv[]) {
    ...
    /* === 优化：mmap 替代 fopen 加载字节码（减少I/O系统调用） === */
    ...
    if (fstat(fd, &st) < 0 || st.st_size <= 0) {
        close(fd);
        fprintf(stderr, "[QVM] 错误: 文件为空或stat失败 %s\n", argv[1]);
        return 1;
    }
    long fsize = st.st_size;

    void *raw = mmap(NULL, fsize, PROT_READ, MAP_PRIVATE | MAP_POPULATE, fd, 0);
    if (raw == MAP_FAILED) {
        close(fd);
        fprintf(stderr, "[QVM] 错误: mmap失败 %s\n", argv[1]);
        return 1;
    }
    ...
    while (pos < code_end) {
        ...
        case OP_STOP: {
            printf("[QVM] 程序退出\n");
            printf("[QVM] 执行完成: %d 周期, %d 门操作\n", vm.cycles, vm.ops);
            munmap(code, fsize); if (vm.state) free(vm.state); return 0;
        }
