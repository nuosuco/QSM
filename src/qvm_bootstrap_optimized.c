 ...
 *   1. mmap replaces fopen for bytecode loading
 *   2. -static + strip for smaller/deployable binary
 *   3. optimized hot-loop branch density
 * ============================================================ */
#include <stdio.h>
...
#define MAX_MEM 128 * 1024 * 1024  /* Reduced from 256MB → 128MB (sufficient for ≤22 qubits) */
...
int main(int argc, char *argv[]) {
    ...
    /* Optimized: mmap bytecode file directly (no malloc/fread overhead) */
    ...
    uint8_t *code = (uint8_t *)mmap(NULL, (size_t)fsize, PROT_READ, MAP_PRIVATE | MAP_POPULATE, fd, 0);
    madvise(code, (size_t)fsize, MADV_SEQUENTIAL);
    close(fd);
    if (code == MAP_FAILED) {
        fprintf(stderr, "[QVM] 错误: mmap 失败\n");
        return 1;
    }
    ...
    while (pos < code_end) {
        ...
        case OP_WHILE_STMT: {
            ...
            lf->condition = cond;
            printf("[QVM] WHILE body=[%d,%d) cond=%d\n", pos, body_end, cond);
            if (cond == 0) {
                printf("[QVM] WHILE 条件为假，跳过循环体\n");
                loop_depth--;
                pos = body_end;
            } else {
                /* 条件为真：循环栈已入，pos 不变（仍指向 body_start），
                   主循环下次迭代从 pos=body_start 开始执行循环体。
                   当 pos 到达 body_end 时（循环迭代检查），重新评估条件决定是否继续。 */
            }
            high_count++;
            break;
        }
        ...
        case OP_FUNC_CALL_STMT: {
            ...
            /* 函数调用：保存返回地址，跳转到函数体 */
            int fidx = (n && n[0]) ? func_find(n) : func_find(name);
            if (fidx >= 0) {
                if (call_depth >= CALL_STACK_MAX) {
                    printf("[QVM] 错误: 调用栈溢出\n");
                    high_count++; break;
